...
public static class KeyboadMapping {
    public static char? ToChar(this Keys key, ModifierKey modifiers = ModifierKey.None) {
        var isShiftDown = (modifiers & ModifierKey.Shift) == ModifierKey.Shift;
        var isAltGrDown = false;

        if (!isShiftDown) {
            isAltGrDown = (modifiers & ModifierKey.Alt) == ModifierKey.Alt || (modifiers & ModifierKey.Ctrl) == ModifierKey.Ctrl;
        }
